...
public record RewardPool(int rolls, List<Entry> entries) {
    public record Entry(ResourceLocation type, int weight, DifficultyRank minimumRank, JsonObject data) {}
    ...
    public static RewardPool parse(JsonObject json) {
        ...
        for (JsonElement element : GsonHelper.getAsJsonArray(json, "entries")) {
            ...
            entries.add(new Entry(ResourceLocation.parse(GsonHelper.getAsString(entry, "type")),
